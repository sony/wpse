...
def save_on_master(state, is_best, output_dir, suffix=None):
    if is_main_process():
        ...
        if suffix is not None:
            spec_path = f"{output_dir}/checkpoint_{suffix}.pt"
            shutil.copyfile(ckpt_path, spec_path)
...
def init_distributed_mode(args):
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:  # torchrun
        ...
        args.dist_backend = "nccl"
        args.dist_url = "env://"
    elif args.dist_url is not None:
        assert args.gpu is not None
        assert args.world_size is not None
        assert args.rank is not None
        assert args.dist_backend is not None
    else:
